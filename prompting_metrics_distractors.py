...
if __name__=="__main__":
    ...
    if "squad" in options.data_name.lower():
        corpus, queries, qrels = download_squad()
    else:
    ...
    # HERE WE CONFUSE THE MODEL
    wrong_answers = filler_targets+questions if filler_targets else questions
    results = calculate_metrics(options.model_name, questions, targets, prompts, options.template, wrong_answers=wrong_answers, k = options.k, batch_size=options.batch_size, embeddings=embeddings_path)
    
    # save the results
    os.makedirs(save_path, exist_ok=True)
    report(f"Saving to {save_path}")
    with open(f'{save_path}/prompt_geometry_w_distractors.json', 'w') as f:
