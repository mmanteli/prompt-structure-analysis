...
for dataset in "mteb/RTE3"; do   
    for model in "${MODELS[@]}"; do
        for lang in "en" "fr"; do
            for template in "Instruct-Query"; do
                CMD=(python prompting_metrics_sts.py \
                    --model=$model \
                ...
                wait_for_space
                echo "${model}:${dataset}:${lang}:${split}_${template}"
                sbatch --job-name="prompt_metrics/${model_safe_name}_${data_safe_name}:${lang}:${split}_${template}" -t 0:19:59 slurm_run_command_gpu.sh "${CMD[@]}"
            done
        done
    done
done
...
split="test"
for dataset in "mteb/stsbenchmark-sts"; do   
    for model in "${MODELS[@]}"; do
        for template in "Instruct-Query"; do
            CMD=(python prompting_metrics_sts.py \
                --model=$model \
            ...
            sbatch --job-name="prompt_metrics/${model_safe_name}_${data_safe_name}:${split}_${template}" -t 0:19:59 slurm_run_command_gpu.sh "${CMD[@]}"
        done
    done
done
